...
- Scripts
	...
	- {{youtube-timestamp 03:02}}
		- | {{youtube-timestamp 03:02}} |
		  ...
		  | This is what I end up with. | 이게내 최종적으로 만들어낸 것입니다. |
	...
	- {{youtube-timestamp 05:51}}
		- | {{youtube-timestamp 05:51}} |
		  ...
		  | So I really felt like I, I have a lot going on in my brain. | 따라서 제 머릿속에는 많은 생각이 있었지만 |
	...
	- {{youtube-timestamp 09:33}}
		...
		- | {{youtube-timestamp 10:39}} |
		  ...
		  | Yeah each source gets a row and I, each source has a process related to it. | 네, 각 소스마다 한 행(row)을 할당하고, 각 소스에는 관련된 프로세스가 있습니다. |
		- | {{youtube-timestamp 11:04}} |
		  | So the first step is outlining the second step is referencing and that creates additional linkages in the graph to other articles that, which is super useful for understanding sort of the layout of my literature. | 첫 번째 단계는 아웃라인을 그리는 것이고 두 번째 단계는 참조하기 위한 것입니다. 그리고 그것이 그래프에서 다른 논문들과의 추가적인 연결을 만들어내며, 이는 나의 참고문헌의 레이아웃을 이해하는 데에 매우 유용합니다. |
		  | I annotate and then I recap everything in a few simple sentences. | 제가 각주를 달고 나서는 모든 것을 몇 가지 간단한 문장으로 요약합니다. |
		  | So that's sort of bringing things into the library finding and adding sources. | 그래서 이것은 종류에 따라서 작업물을 라이브러리에 가져오고 소스를 찾아 추가하는 것입니다. |
		  | And again, I use Google Scholar for this and also connected papers which I'll just very briefly show you. | 그리고 다시 말하지만, 나는 이 작업을 위해 Google Scholar와 connected papers를 사용하는데, 이를 간단히 보여드리겠습니다. |
		- | {{youtube-timestamp 11:34}} |
		  | not yet. So I don't like to use a Zotero because it overwhelms me when I'm not sure how I'm going to actually use these articles when I'm citing them. | 아직은 사용하지 않아요. Zotero를 사용하는 것은 좋아하지 않아요. 인용할 때 이 논문들을 어떻게 실제로 사용할지 잘 모를 때 압도되기 때문이에요. |
		  | So I like to wait until towards the end of my writing process to put all of the different things that I'm citing into Zotero so that it doesn't get mixed together with things that I'm definitely citing and things that I'm just looking through because I might be interested in citing it for some reason or drawing from it. | 그래서 나는 인용하는 모든 다른 것들을 내 작성 과정의 끝쪽까지 기다리고, 인용하는 것과 그냥 참고하고 싶은 이유로 인용하거나 참고하려고 살펴보는 것들이 섞이지 않도록 Zotero에 넣으려고 해. |
		- | {{youtube-timestamp 12:01}} |
		  | So I think this is also a good lesson. | 그러니까, 저는 이것도 좋은 교훈이라고 생각합니다. |
		  | I don't know if it's intentional. | 의도한 것인지 모르겠습니다. |
		  | So you say once you're done with basically your dissertation, that's or at least a part you will then look into the terror not now like you're not trying to change the wheel of a car while driving. | 그래서 기본적으로 논문이 끝나면, 최소한 일부분을 이제 공포로 바라보지 않으면서하는 거죠. 마치 운전 중에 차의 바퀴를 바꾸려고 하는 것과 같지 않게요. |
		  | Basically same thing with the process you're not filling too much to upend your process as you're using it. | 기본적으로 같은 맥락에서, 프로세스를 사용하고 있는 동안에는 너무 많은 것을 채우지 않는 것이 좋습니다. |
		  | I think that's also an important lesson for, at least for myself. | 저는 이것 또한 중요한 교훈이라고 생각합니다. 적어도 제 자신에게는요. |
		- | {{youtube-timestamp 12:31}} |
		  | Yeah so I like to, I, you'll see in my graph I tend to keep things pretty simple. I don't there's not a whole lot of bells and whistles. I like to wait until the end because you know, a lot of things will just become complicated sort of emergent systems tend to become complex just how they are. So I try to keep things intentionally simple so that the more complicated system emerges on its own trying to find something here so that I could show you what it looks like when I'm drafting just conflict. | 그래서 나는 그래프에서 보듯이 일반적으로 간단하게 유지하려고 합니다. 그리고 많은 것들은 그냥 복잡해 질 것입니다. 등불과 선물 상자가 그렇게 복잡한 것 같진 않아요. 그래서 제가 끝까지 기다리려고 합니다. 왜냐하면 많은 것들이 단순히 복잡해지고 복잡한 체계는 그저 그렇게 나타나는 경향이 있거든요. 그래서 일부러 단순하게 유지하여 더 복잡한 체계가 자연스럽게 나타날 수 있도록 하려고 합니다. 단지 갈등이 생기기만 하면 어떻게 보이는지 보여줄 수 있는 것을 찾으려고 노력 중이에요. |
		...
		- **Ramses Oudt** {{youtube-timestamp 42:26}}
